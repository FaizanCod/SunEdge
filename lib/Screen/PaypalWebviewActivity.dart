...
class StatePayPalWebview extends State<PaypalWebview> {
                          ...
                          if (data.split("=")[0].toLowerCase() == "tx" ||
                            ...
                            if (widget.from == "order") {
                              if (request.url.startsWith(PAYPAL_RESPONSE_URL)) {
                                ...
                                AddTransaction(txid, widget.orderId!, SUCCESS, 'Order placed successfully', true);
                                //placeOrder(txid);
  ...
  Future<void> placeOrder(String tranId) async {
    ...
    try {
      ...
      request.fields[FINAL_TOTAL] =  usedBal>0?totalPrice.toString():isStorePickUp == "false"?(totalPrice+delCharge).toString():totalPrice.toString();
  ...
  Future<void> AddTransaction(String tranId, String orderID, String status,
    ...
    try {
      var parameter = {
        ...
        TYPE: payMethod,
        TXNID: tranId,
        AMOUNT:  usedBal>0?totalPrice.toString():isStorePickUp == "false"?(totalPrice+delCharge).toString():totalPrice.toString(),
        STATUS: status,
        MSG: msg
      };
